...
<html>
	...
	<body>
		...
		<h1><s:text name="label.estudiantes.asignatura"/> "<s:property value="#session.nombre"/>" <s:text name="label.para"/> "<s:property value="%{ asignatura.nombre }"/>"</h1>
		...
		<table class="borderAll">
			...
			<s:iterator value="alumnos" status="status">
		        <tr class="<s:if test="#status.even">even</s:if><s:else>odd</s:else>">
		            ...
		            <s:url id="urlVerDetallesAlumno" action="profesor!verDetallesAlumno" escapeAmp="false">
		            	<s:param name="dniAlumno" value="%{ dni }"/>
		            	<s:param name="idAsignatura" value="idAsignatura"/>
		            </s:url>
		            <td class="nowrap"><a href="<s:property value="#urlVerDetallesAlumno"/>"><s:property value="%{ nombre }"/></a></td>		            

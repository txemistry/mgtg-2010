...
public class PtDAO implements IptDAO 
{
	...
	{
		Transaction tx = session.beginTransaction();
        Alumno alumno = (Alumno) session.createQuery("from Alumno where alum_dni = " + dniAlumno).uniqueResult();
		Set<Asignatura> asignaturas = alumno.getAsignaturas();
		Set<Integer>idProfesores = new HashSet<Integer>();
		for(Asignatura asig:asignaturas)
		{
			idProfesores.add(asig.getProfesor().getId());
		}

		List<Profesor> profs = (List<Profesor>) session.createQuery("from Profesor where prof_id in (:idProfesores) order by prof_nom desc").setParameterList("idProfesores", idProfesores).list();
	...
	{
		Transaction tx = session.beginTransaction();
        Alumno alumno = (Alumno) session.createQuery("from Alumno where alum_dni = " + dni).uniqueResult();
        
        if(alumno == null)
        {
        	//el alumno no existe y se puede guardar
        	Alumno alumnoNuevo = new Alumno(dni, password, nombre, telefono);
        	session.save(alumnoNuevo);
        }
        else
        {
